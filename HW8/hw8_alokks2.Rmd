...
The regresion line seems to be passing through average values for `Car` and for `Truck` and `Both` it seems to be underestimating a little.
...
For a significance level of `0.01`; the mixture model is better.
...
patient1 = data.frame(Charges=2000,Pressure=80,Care="high",Race="white")
patient2 = data.frame(Charges=2000,Pressure=81,Care="high",Race="white")
mean(predict(days_mix2,data=patient2)-predict(days_mix2,data=patient1))
```
Meaning, there wont be any increase in number of days.
...
\beta_4 != 0, \beta_5 != 0 and \beta_7 != 0
...
for(i in 1:num_sims){
  ...
  lm_t[i] = glance(lm(formula = values ~ groups, data = ex4))$statistic
  lm_p[i] = glance(lm(formula = values ~ groups, data = ex4))$p.value
