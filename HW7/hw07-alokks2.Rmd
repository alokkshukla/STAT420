...
We notice that p-value of this test is significant (>0.05), thus we fail to reject the null hypothesis for 0.10 significance level. And we fail to conclude if recipies had any effect on ratings. A small F-value suggests that there is a lot variance inside samples, even though we can see there is difference between means across samples.
...
x = c(0)
for (i in 1:100){
  p_vals = replicate(n = 1000, sim_anova2(n=i,mu_a = -1, mu_b = 0, mu_c = 1, 
                                     sigma = 1, stat = FALSE))
  y = c(y,mean(p_vals < 0.0025))
  if(is.na(y[i])){
    y[i] = 0
  }
  x=c(x,i)
...
plot(x[2:length(x)],y[2:length(x)],type="n", main="Various Sample sizes vs Mean p-val", xlab = "Size of sample", ylab = "Mean p-val",xaxt="n",yaxt="n")
...
We observe that for sample size = 13, we get a p-val greater than 0.9; thus 13 is the chosen sample size.
...
for (a in x){
  b = 10-a;
  p_vals = replicate(n = 500, sim_anova(mu_a = 0, mu_b = 2, n_a = a, n_b = b,
                                     sigma = 1, stat = FALSE))
 
  
  mean_p_vals[a]=mean(p_vals < 0.0025)
 
}

plot(x,mean_p_vals,type="n", main="Various Sample size combinations vs Mean p-val", xlab = "Size of one sample, other's size = 10 - label", ylab = "Mean p-val",xaxt="n")
axis(1, at = x, las=2)
