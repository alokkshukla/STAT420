...
```{r}
model_crim=lm(medv~crim,data=train)
...
```{r}
model3=lm(medv~crim+zn+indus+nox+rm+age+dis+rad+tax+ptratio+black+lstat,data=train)
(trainRMSE = sqrt(mean(model3$residuals^2)))
test_resid = abs(predict(model3,newdata = test)-test$medv)
(testRMSE = sqrt(mean(test_resid^2)))

model4=lm(medv~crim+zn+indus+nox+rm+age+dis+tax+ptratio+black+lstat,data=train)
(trainRMSE = sqrt(mean(model4$residuals^2)))
test_resid = abs(predict(model4,newdata = test)-test$medv)
(testRMSE = sqrt(mean(test_resid^2)))

model5=lm(medv~crim+indus+nox+rm+age+dis+tax+ptratio+black+lstat,data=train)
